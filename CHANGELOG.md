...
[2026-08-08] v0.3.1 — Buy List: default selection changed from all-checked/qty-10 to empty — merchant now picks manually or taps "Smart Pick" (auto-selects using stock urgency, sales velocity, customer requests, margin, and blended historical order quantity). Added search bar and "Clear List" (deselect all) — improvement
[2026-08-08] v0.4.0 — FlowChat: added real action execution (was pure Q&A before). Navigation ("open Inventory"), settings control (theme, dark/light/system mode, voice on/off) run immediately. Restock and discount commands require an in-chat Confirm/Cancel step first since they touch money/pricing. Mark-out-of-stock runs immediately (reversible, non-financial). Added persisted, reopenable, individually-deletable chat history (per-store, stored on-device) — feature
[2026-08-08] v0.5.0 — Purchase Import Code system: approving a Buy List now generates a short code (e.g. PO-4F7QXK), saved to Purchase Orders history alongside the item list. New "Import with Code" tab in Inventory > Import, and a matching entry point in Purchase Orders history — enter the code to bring those exact items into stock. Codes are single-use (validated against existing code, match, and not-already-imported before allowing import) — feature
[2026-08-08] v0.5.1 — FlowChat: widened command phrasing (normalizes "please"/"can you"/"I want to" wrapping, more verb variants for restock/discount/out-of-stock/navigation), added soft product-lookup phrasings ("check X", "X stats") that fall through to normal Q&A instead of assuming a miss means "add this product." Confirmed Buy List default behavior (rows visible, nothing pre-selected) already matches what was asked — no change needed there — improvement
