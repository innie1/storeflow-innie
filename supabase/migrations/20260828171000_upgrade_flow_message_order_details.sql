-- Upgrade Flow-created merchant orders with structured payment/fulfilment metadata.
-- The function remains atomic and authenticated-store-member only.

drop function if exists public.merchant_create_flow_message_order(uuid, text, text, jsonb, text, text, text, text);

create function public.merchant_create_flow_message_order(
  p_store_id uuid,
...
begin
  ...
  v_details := case
    ...
    when jsonb_typeof(p_details) = 'object' then p_details
    else '{}'::jsonb
  end;
  ...
  v_service_metadata := jsonb_build_object(
    'source', 'flow_message',
    'created_by', auth.uid(),
    'created_at', now()
  ) || v_details;
...
revoke all on function public.merchant_create_flow_message_order(uuid, text, text, jsonb, text, text, text, text, jsonb) from public;
revoke all on function public.merchant_create_flow_message_order(uuid, text, text, jsonb, text, text, text, text, jsonb) from anon;
grant execute on function public.merchant_create_flow_message_order(uuid, text, text, jsonb, text, text, text, text, jsonb) to authenticated;
grant execute on function public.merchant_create_flow_message_order(uuid, text, text, jsonb, text, text, text, text, jsonb) to service_role;
