...
export interface SmartBuyListItem {
  productId: string; name: string; quantity: number; unitCost: number; totalCost: number; score: number;
  reason: string; sold30: number; sold90: number; currentStock: number; daysCover: number | null; typicalRestockQty: number;
}
...
function scoreProduct(store: StoreData, product: Product) {
  ...
  const s30=salesFor(store,product.id,30), s90=salesFor(store,product.id,90);
  const sold30=s30.reduce((a,s)=>a+n(s.quantity),0), sold90=s90.reduce((a,s)=>a+n(s.quantity),0);
  const revenue90=s90.reduce((a,s)=>a+n(s.total),0), profit90=s90.reduce((a,s)=>a+n(s.profit),0);
  ...
  const velocity=(sold30/30)*0.65+(sold90/90)*0.35;
  ...
  const score=Math.min(40,velocity*8)+Math.min(18,revenue90>0?Math.log10(revenue90+1)*5:0)+Math.min(12,profit90>0?Math.log10(profit90+1)*3.2:0)
    +(product.quantity<=0?20:product.quantity<=reorderLevel?14:daysCover!==null&&daysCover<targetDays?10:0)
    +Math.min(10,restocks.length*1.5+(restocks.length>=3?3:0))+(lastSale<=7?8:lastSale<=14?6:lastSale<=30?3:0);
...
export function buildSmartBuyList(store: StoreData, budget?: number): SmartBuyListResult {
  ...
  for(const c of candidates){
    const unitCost=Math.max(0,n(c.product.costPrice)); if(unitCost<=0||remaining<unitCost) continue;
    let quantity=Math.min(c.suggested,Math.max(1,c.typicalRestockQty*2)); quantity=Math.min(quantity,Math.floor(remaining/unitCost)); if(quantity<=0) continue;
    const totalCost=quantity*unitCost; items.push({productId:c.product.id,name:c.product.name,quantity,unitCost,totalCost,score:Math.round(c.score*10)/10,reason:c.reason,sold30:c.sold30,sold90:c.sold90,currentStock:c.product.quantity,daysCover:c.daysCover,typicalRestockQty:c.typicalRestockQty}); remaining-=totalCost;
  }
...
export function smartBuyListText(result: SmartBuyListResult) {
  ...
  return ['**Smart Buy List**',result.budget?`Budget: **₦${Math.round(result.budget).toLocaleString()}**`:`Budget: **₦${Math.round(result.availableBalance).toLocaleString()} available**`,'',...result.items.map((item,index)=>`${index+1}. **${item.name}** × ${item.quantity} — ₦${Math.round(item.totalCost).toLocaleString()}\n   ${item.reason}`),'',`Estimated total: **₦${Math.round(result.estimatedCost).toLocaleString()}**`].join('\n');
}
