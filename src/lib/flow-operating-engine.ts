...
export type OperatingIntent =
  | 'sell' | 'restock' | 'add_product' | 'undo' | 'store_overview' | 'inventory'
  | 'sales' | 'profit' | 'best_sellers' | 'slow_products' | 'pricing' | 'customers'
  | 'expenses' | 'improvement' | 'navigation' | 'settings' | 'product_lookup' | 'help' | 'unknown';
...
const STOP = new Set(['the','a','an','my','me','please','product','products','item','items','store','stock','inventory','now','today','for','of','to','on','is','are','what','whats','show','tell','about','do','i','can','you','give','get','some','something','thing','things','with','and','or','in','at','from','this','that']);

function norm(v: string) {
  return v.toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/&/g, ' and ').replace(/[^a-z0-9%₦]+/g, ' ').trim().replace(/\s+/g, ' ');
}
function tokens(v: string) { return norm(v).split(' ').filter(Boolean).filter(x => !STOP.has(x)); }
function distance(a: string, b: string) {
  const aa = norm(a), bb = norm(b), prev = Array.from({ length: bb.length + 1 }, (_, i) => i);
  for (let i = 1; i <= aa.length; i++) { const cur = [i]; for (let j = 1; j <= bb.length; j++) cur[j] = aa[i - 1] === bb[j - 1] ? prev[j - 1] : Math.min(prev[j - 1] + 1, prev[j] + 1, cur[j - 1] + 1); for (let j = 0; j < cur.length; j++) prev[j] = cur[j]; }
  return prev[bb.length];
}
function similarity(a: string, b: string) {
  const aa = norm(a), bb = norm(b); if (!aa || !bb) return 0; if (aa === bb) return 1;
  if (aa.includes(bb) || bb.includes(aa)) return .95;
  const ta = new Set(tokens(aa)), tb = new Set(tokens(bb));
  const overlap = [...ta].filter(x => tb.has(x)).length; const union = new Set([...ta, ...tb]).size || 1;
  const fuzzy = 1 - distance(aa, bb) / Math.max(aa.length, bb.length);
  return Math.max(overlap / union * .94, fuzzy * .84);
}
...
function cleanProductPhrase(s: string) {
  return s.replace(/^\s*(?:please\s+)?(?:sell|sold|record\s+(?:a\s+)?sale|restock|receive|stock\s+up|add|buy|increase|reduce)\s*/i, '')
    .replace(/^\s*(?:me|my|the)\s+/i, '').replace(/\b(?:qty|quantity|units?|stock)\s*[:=]?\s*\d+/ig, '')
    .replace(/\b(?:at|for)\s+₦?[\d,]+/ig, '').trim();
}
...
function extractQty(s: string): number {
  const patterns = [
    /^\s*(\d+(?:\.\d+)?)\s+(?:x\s*)?/i,
    /\bx\s*(\d+(?:\.\d+)?)\b/i,
    /\b(?:qty|quantity|units?|stock)\s*[:=]?\s*(\d+(?:\.\d+)?)\b/i,
    /\b(?:sell|sold|restock|receive|add|buy)\s+(?:me\s+)?(\d+(?:\.\d+)?)\b/i,
  ];
  for (const r of patterns) { const m = s.match(r); if (m) return Math.max(1, Math.round(Number(m[1]))); }
  return 1;
}

function splitItems(text: string) {
  const body = text.replace(/^\s*(?:please\s+)?(?:sell|sold|restock|receive|add|buy)\s+/i, '').trim();
  return body.split(/\s*,\s*|\s+and\s+/i).map(x => x.trim()).filter(Boolean);
}

function parseItems(store: StoreData, text: string): FlowLineItem[] {
  const items: FlowLineItem[] = [];
  for (const part of splitItems(text)) {
    const q = extractQty(part); const phrase = cleanProductPhrase(part).replace(/^\d+(?:\.\d+)?\s+(?:x\s*)?/i, '').trim();
    const match = resolveProduct(store, phrase);
    if (match && match.score >= .76) items.push({ product: match, quantity: q });
  }
  return items;
}

function nav(text: string): TabId | undefined {
  const q = norm(text); if (!/^(open|go to|take me to|navigate to|switch to|show me)\b/.test(q)) return undefined;
  const pairs: [string, TabId][] = [['dashboard','dashboard'],['home','dashboard'],['inventory','inventory'],['stock','inventory'],['products','inventory'],['sales','sales'],['sell','sales'],['history','history'],['expenses','expenses'],['settings','settings'],['orders','orders'],['customers','customers'],['suppliers','suppliers'],['goals','goals'],['staff','staff'],['cash drawer','cash-drawer'],['wishlist','wishlist']];
  return pairs.find(([w]) => q.includes(w))?.[1];
}

export function understand(store: StoreData, raw: string, lastProduct?: Product | null, lastIntent?: OperatingIntent): OperatingPlan {
  const text = raw.trim(), q = norm(text); const empty = { items: [], confidence: 0, reason: 'empty input' } as const;
  if (!q) return { intent: 'unknown', ...empty };
  ...
  if (/\b(?:dark|light|system)\s+(?:theme|mode)\b|\b(?:turn|switch)\s+(?:on|off)\s+(?:voice|sound)\b/.test(q)) return { intent: 'settings', confidence: .98, items: [], reason: 'setting command' };
  ...
  if (/\b(?:improve|grow|growth|ideas?|advice|recommendations?|what should i do)\b/.test(q)) return { intent: 'improvement', confidence: .97, items: [], reason: 'business advice' };
  ...
  if (action) {
    const items = parseItems(store, text);
    if (!items.length && lastProduct && /^(?:and|also)\b/i.test(text)) return { intent: action, confidence: .9, items: [{ product: { product: lastProduct, score: 1, matchedBy: 'exact' }, quantity: extractQty(text) }], product: { product: lastProduct, score: 1, matchedBy: 'exact' }, quantity: extractQty(text), reason: 'follow-up using last product' };
    return { intent: action, confidence: items.length ? Math.min(.99, .82 + items.length * .05) : .55, items, product: items[0]?.product, quantity: items[0]?.quantity, reason: items.length > 1 ? 'batch operation' : 'single operation' };
  }

  const direct = resolveProduct(store, text);
  if (direct && direct.score >= .76) return { intent: 'product_lookup', confidence: direct.score, items: [], product: direct, reason: 'product match' };
  if (/^(?:and|also)\b/i.test(text) && lastProduct) return { intent: lastIntent === 'restock' ? 'restock' : lastIntent === 'sell' ? 'sell' : 'product_lookup', confidence: .86, items: [], product: { product: lastProduct, score: 1, matchedBy: 'exact' }, quantity: extractQty(text), reason: 'context follow-up' };
...
export function storeAnalysis(store: StoreData) {
  const products = (store.products || []).filter(p => !p.discontinued), s7 = recent(store, 7), s30 = recent(store, 30), threshold = store.managerSettings?.minStockThreshold ?? store.managerSettings?.criticalStockThreshold ?? 5;
  const units = new Map<string, number>(), rev = new Map<string, number>();
  ...
  const out = products.filter(p => p.quantity <= 0), low = products.filter(p => p.quantity > 0 && p.quantity <= threshold);
  const dead = products.filter(p => p.quantity > 0 && !units.get(p.id));
  const underpriced = products.filter(p => p.costPrice > 0 && ((p.sellingPrice - p.costPrice) / p.sellingPrice) < .12);
  ...
  const revenue7 = s7.reduce((a,s) => a+s.total,0), profit7 = s7.reduce((a,s) => a+s.profit,0), revenue30=s30.reduce((a,s)=>a+s.total,0), profit30=s30.reduce((a,s)=>a+s.profit,0);
  const expenses30=(store.expenses||[]).filter(e=>new Date(e.date).getTime()>=Date.now()-30*86400000).reduce((a,e)=>a+e.amount,0);
  const debt=(store.pendingPayments||[]).reduce((a,p)=>a+Math.max(0,p.amount-p.amountPaid),0);
  return { products, out, low, dead, underpriced, top, revenue7, profit7, revenue30, profit30, expenses30, debt, threshold };
}

export function responseFor(store: StoreData, plan: OperatingPlan) {
  const a = storeAnalysis(store); const product = plan.product?.product;
  switch (plan.intent) {
    case 'store_overview': {
      const health = Math.max(0, Math.min(100, Math.round(70 + (a.revenue7 > 0 ? 8 : -8) - a.out.length * 4 - a.underpriced.length * 2)));
      return `Your store is at about **${health}/100** right now.\n\nRevenue (7 days): **${money(a.revenue7)}**\nProfit (7 days): **${money(a.profit7)}**\n${a.low.length + a.out.length ? `⚠️ **${a.low.length + a.out.length} products** need attention.` : '✅ Inventory looks stable.'}\n${a.underpriced.length ? `⚠️ **${a.underpriced.length} products** have thin margins.` : '✅ No obvious thin-margin products.'}\n\n**My priority:** ${a.out.length ? `restock ${a.out[0].name}` : a.low.length ? `restock ${a.low[0].name}` : a.underpriced.length ? `review ${a.underpriced[0].name}'s price` : 'keep monitoring sales and stock'}.`;
    }
    case 'inventory': {
      const list = [...a.out, ...a.low].slice(0, 8); return list.length ? `Products needing stock attention:\n${list.map((p,i)=>`${i+1}. **${p.name}** — ${p.quantity} left`).join('\n')}\n\n${a.out.length ? `🔴 ${a.out.length} out of stock.` : ''}` : 'Your active products are not currently below the low-stock threshold.';
    }
    case 'best_sellers': return a.top.length ? `Your best sellers in the last 30 days:\n${a.top.map((x,i)=>`${i+1}. **${x.product.name}** — ${x.units} units / ${money(x.revenue)}`).join('\n')}` : 'I do not have enough sales history yet.';
    case 'slow_products': return a.dead.length ? `These products have stock but no recorded sale in 30 days:\n${a.dead.slice(0,8).map(p=>`• **${p.name}** — ${p.quantity} left`).join('\n')}\n\nI would stop buying more until they move.` : 'Nothing is completely idle right now.';
    ...
    case 'pricing': return product ? `**${product.name}** sells for ${money(product.sellingPrice)} with cost ${money(product.costPrice)}. Gross margin: **${product.sellingPrice ? Math.round((product.sellingPrice-product.costPrice)/product.sellingPrice*100) : 0}%**.` : a.underpriced.length ? `I found **${a.underpriced.length} products** with margins below 12%:\n${a.underpriced.slice(0,6).map(p=>`• ${p.name} — ${money(p.sellingPrice)} sell / ${money(p.costPrice)} cost`).join('\n')}` : 'I do not see an obvious thin-margin pricing problem.';
    ...
    case 'improvement': {
      const items = [...a.out.slice(0,3).map(p=>`Restock **${p.name}** — it is out of stock.`), ...a.underpriced.slice(0,2).map(p=>`Review **${p.name}** — margin is thin.`), ...a.dead.slice(0,2).map(p=>`Reduce buying **${p.name}** — stock is not moving.`)];
      return items.length ? `Here are my priorities today:\n${items.slice(0,5).map((x,i)=>`${i+1}. ${x}`).join('\n')}\n\n**My first recommendation:** ${items[0]}` : 'Your store does not show a major operational problem from the data I can see. Keep monitoring stock, margins and sales.';
    }
...
export function aliasSuggestion(store: StoreData, query: string) {
  const match = resolveProduct(store, query); if (!match || match.score >= .9) return null;
  return `I think you mean **${match.product.name}**. Is that right?`;
}
