...
export default function FlowChat({ store, onClose, onNavigate, onUpdate }: FlowChatProps) {
  ...
  const [lastIntent, setLastIntent] = useState<OperatingIntent | undefined>();
  ...
  const analysis = useMemo(() => storeAnalysis(store), [store]);
  useEffect(() => { scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: 'smooth' }); }, [messages]);
  useEffect(() => { localStorage.setItem('storeflow_flow_voice', voiceOn ? '1' : '0'); }, [voiceOn]);
  useEffect(() => {
    if (messages.length <= 1) return;
    setSessions(prev => {
      const next = [...prev]; const item = { id: sessionId, title: (messages.find(m => m.from === 'you')?.text || 'New chat').slice(0, 42), updatedAt: new Date().toISOString(), messages };
      const i = next.findIndex(s => s.id === sessionId); if (i >= 0) next[i] = item; else next.unshift(item); saveSessions(storeKey, next); return next;
    });
  ...
  const executeSales = (items: FlowLineItem[]) => {
    ...
    for (const item of items) {
      const p = next.products.find(x => x.id === item.product.product.id);
      if (!p) continue;
    ...
    onUpdate(next);
    const remaining = done.map(x => { const name = x.replace(/^\d+\s+/, ''); return next.products.find(p => p.name === name); }).filter(Boolean) as Product[];
    if (items.length === 1) {
      const p = next.products.find(x => x.id === items[0].product.product.id)!; setLastProductId(p.id);
      flow(`Done — sold **${items[0].quantity} ${p.name}**.\nTotal: **${money(total)}**\nProfit: **${money(profit)}**\nStock: **${p.quantity} remaining**${p.quantity === 0 ? '\n⚠️ Now out of stock.' : ''}`);
    } else {
      flow(`Done — **${done.length} products sold**.\n${done.map(x => `• ${x}`).join('\n')}\n\nTotal: **${money(total)}**\nProfit: **${money(profit)}**`);
    }
  ...
  const executeRestock = (items: FlowLineItem[]) => {
    ...
    for (const item of items) {
      ...
      next = receiveStock(next, [{ productId: p.id, quantity: item.quantity, costPrice: p.costPrice }], 'balance', 'FlowChat'); done.push(`${item.quantity} ${p.name}`);
    }
    onUpdate(next); setLastProductId(items[0].product.product.id);
    flow(`Done — added stock for **${done.length} products**.\n${done.map(x => `• ${x}`).join('\n')}`); showToast('Stock updated by Flow', 'success');
  };

  const finishAdd = () => {
    if (!addDraft || addDraft.costPrice == null || addDraft.sellingPrice == null || addDraft.quantity == null || !addDraft.category) { flow('I still need cost, selling price, quantity and category.'); return; }
    rememberUndo(); const next = addProduct(store, addDraft); onUpdate(next); setLastProductId(next.products[next.products.length - 1]?.id || null); setAddDraft(null);
    flow(`Added **${addDraft.name}** to Inventory.\nStock: ${addDraft.quantity}\nSelling: ${money(addDraft.sellingPrice)}\nCost: ${money(addDraft.costPrice)}.`); showToast('Product added', 'success');
  };
  ...
  const handleAddWizard = (text: string) => {
    ...
    else if (addStep === 'category') { d.category = text; setAddStep('confirm'); flow(`Add **${d.name}** — cost ${money(d.costPrice || 0)}, sells ${money(d.sellingPrice || 0)}, ${d.quantity} units, category ${d.category}?`, [{ label: 'Add', onClick: finishAdd }, { label: 'Cancel', onClick: () => { setAddDraft(null); flow('Cancelled.'); } }]); }
    else if (/^(yes|y|add|confirm|ok|okay)$/i.test(text)) finishAdd(); else { setAddDraft(null); flow('Cancelled.'); }
    setAddDraft(d);
  };

  const ask = (raw: string) => {
    const text = clean(raw); if (!text) return; you(text);
    if (/^(undo|undo that|reverse that|take that back)$/i.test(text)) {
      if (!lastUndo) { flow('There is nothing recent I can undo.'); return; }
      const previous = lastUndo; setLastUndo(null); onUpdate(previous); flow('Done — I reversed my last change.'); return;
    }
    ...
    const plan = understand(store, text, lastProduct, lastIntent);
    setLastIntent(plan.intent);
    if (plan.product) setLastProductId(plan.product.product.id);
    ...
    if (plan.intent === 'settings') {
      ...
      if (/turn on (voice|sound)/i.test(text)) { setVoiceOn(true); flow('Voice is on.'); return; }
      if (/turn off (voice|sound)/i.test(text)) { setVoiceOn(false); flow('Voice is off.'); return; }
    ...
    if (plan.intent === 'product_lookup' && plan.product && plan.product.score < .9) {
      const p = plan.product.product; flow(`I think you mean **${p.name}**. Is that right?`, [{ label: 'Yes — use it', onClick: () => { setLastProductId(p.id); flow(`Got it. I'll remember **${p.name}** for this chat.`); } }]); return;
    }
    if (plan.intent === 'product_lookup' && plan.product) { flow(responseFor(store, plan)); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2"><Mascot size={28} /><h3 className="text-base font-display font-bold">Flow</h3><span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary">Local Brain</span></div>
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => lastUndo && (onUpdate(lastUndo), setLastUndo(null), flow('Undone.'))} disabled={!lastUndo} className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-surface-2/60 disabled:opacity-30" title="Undo"><RotateCcw className="w-4 h-4" /></button>
          ...
          <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-surface-2/60"><X className="w-5 h-5" /></button>
        </div>
      </div>
      ...
      {showHistory && <div className="fixed inset-0 z-[60] bg-black/50 flex items-end sm:items-center justify-center" onClick={() => setShowHistory(false)}>
        <div className="w-full sm:max-w-sm max-h-[75vh] bg-background border border-border rounded-t-2xl sm:rounded-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="overflow-y-auto">{sessions.length === 0 ? <p className="text-xs text-muted-foreground text-center py-8">No past chats yet.</p> : sessions.map(s => <div key={s.id} className="flex items-center gap-2 px-4 py-3 border-b border-border/60 cursor-pointer hover:bg-surface-2/40" onClick={() => openSession(s)}><div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate">{s.title}</p><p className="text-[11px] text-muted-foreground">{new Date(s.updatedAt).toLocaleString()}</p></div><button onClick={e => { e.stopPropagation(); deleteSession(s.id); }}><Trash2 className="w-4 h-4 text-muted-foreground" /></button></div>)}</div>
        </div>
      </div>}

      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
        {messages.map(m => <div key={m.id} className="flex flex-col gap-1.5" style={{ alignItems: m.from === 'you' ? 'flex-end' : 'flex-start' }}><div className={`max-w-[90%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed whitespace-pre-line ${m.from === 'flow' ? 'bg-surface-2/60 text-foreground rounded-bl-sm' : 'bg-primary text-primary-foreground rounded-br-sm'}`}>{m.text}</div>{m.actions && <div className="flex gap-2 flex-wrap">{m.actions.map(a => <button key={a.label} onClick={a.onClick} className="px-3 py-2 rounded-full text-xs font-display font-semibold border border-primary/30 bg-primary/10 text-primary">{a.label}</button>)}</div>}</div>)}
        {!addDraft && QUICK.map(q => <button key={q} onClick={() => ask(q)} className="self-start px-3 py-2 rounded-full text-xs font-display font-semibold border border-border bg-surface-2/30">{q}</button>)}
      </div>
