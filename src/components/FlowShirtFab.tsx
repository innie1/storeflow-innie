...
export default function FlowShirtFab({ store, onUpdate, onNavigate, currentUser }: Props) {
  ...
  const openFlowMessages = (startVoice = false) => {
    try {
      sessionStorage.setItem('storeflow_open_flow_messages', startVoice ? 'voice' : 'text');
    } catch {}
    onNavigate?.('manager');
    window.setTimeout(() => {
      window.dispatchEvent(new CustomEvent(FLOW_MESSAGE_EVENT, { detail: { startVoice } }));
    }, 0);
  };
    ...
    for (const item of draft) {
      ...
      if (productId) {
        ...
        const newProduct: Product = {
          id: generateId(), name: item.name, costPrice: 0, sellingPrice: price,
          quantity: item.quantity, initialQuantity: item.quantity,
          category: store.storeType || 'others', addedAt: new Date().toISOString(),
          source: 'voice_sale', needsStockSetup: true,
        };
  ...
  const createVoiceProduct = (name: string, sellingPrice: number, costPrice: number, quantity: number): Product => {
    const newProduct: Product = { id: generateId(), name, costPrice: costPrice || 0, sellingPrice, quantity: Math.max(0, quantity || 0), initialQuantity: Math.max(0, quantity || 0), category: store.storeType || 'others', addedAt: new Date().toISOString(), source: 'voice_sale', needsStockSetup: !costPrice };
    const updated = { ...store, products: [...store.products, newProduct] } as StoreData; commit(updated); return newProduct;
  };

  const saveVoiceAlias = (productId: string, alias: string) => {
    const clean = alias.trim().toLowerCase(); if (!clean) return;
      ...
      {open && canSellProducts && (
        <div className="fixed inset-0 z-[90] bg-black/65 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setOpen(false)}>
          <div className="w-full sm:max-w-md max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-background border border-border p-5 space-y-4" onClick={event => event.stopPropagation()}>
            ...
            {draft.length > 0 && <section className="rounded-2xl border border-border bg-card overflow-hidden"><div className="px-4 py-3 border-b border-border"><p className="text-xs font-display font-black">Review list</p></div><div className="divide-y divide-border/70">{draft.map((item, index) => <div key={`${item.raw}-${index}`} className="p-3.5 flex items-center gap-3"><div className="min-w-0 flex-1"><p className="text-sm font-bold truncate">{item.name} × {item.quantity}</p><p className="text-[10px] text-muted-foreground mt-0.5">{item.product ? (item.confidence === 'exact' ? 'Matched inventory' : 'Likely inventory match') : 'New item — price required'}</p></div>{item.product ? <span className="text-xs font-black">₦{Number(item.product.sellingPrice || 0).toLocaleString()}</span> : <div className="w-28 h-9 rounded-lg border border-border bg-surface-2 px-2 flex items-center gap-1"><span className="text-[10px] text-muted-foreground">₦</span><input value={item.priceGuess ?? ''} onChange={event => updateDraftPrice(index, event.target.value)} inputMode="decimal" placeholder="Price" className="w-full min-w-0 bg-transparent outline-none text-xs font-black text-right" /></div>}</div>)}</div><div className="p-3"><button onClick={saveTypedSale} className="w-full h-11 rounded-xl bg-primary text-primary-foreground font-display font-black text-sm">Save Sale</button></div></section>}
            <section className="rounded-2xl border border-border bg-card p-3.5 space-y-2"><div className="flex items-center gap-2"><Mic className="w-4 h-4 text-primary" /><p className="text-xs font-display font-black">Voice mode</p></div><SimpleVoiceSell products={activeProducts} onConfirmSale={confirmVoiceSale} onConfirmMultiSale={confirmVoiceMultiSale} onCreateProduct={createVoiceProduct} onSaveAlias={saveVoiceAlias} /></section>
          </div>
        </div>
      )}
    </>
