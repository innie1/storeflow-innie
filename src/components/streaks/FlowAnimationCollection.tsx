...
import { FLOW_ANIMATIONS, getUnlockedFlowAnimations, playFlowAnimation, FlowAnimationId } from '@/lib/flow-animation-unlocks';
    ...
    <div className="fixed inset-0 z-[80] bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div className="w-full sm:max-w-md max-h-[82vh] overflow-hidden rounded-t-3xl sm:rounded-3xl border border-border bg-background shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div><h2 className="font-display font-black text-lg">Flow animations</h2><p className="text-xs text-muted-foreground">Tap an unlocked animation to make Flow perform it.</p></div>
        ...
        <div className="grid grid-cols-2 gap-3 p-4 overflow-y-auto max-h-[65vh]">
          {FLOW_ANIMATIONS.map(animation => {
            ...
            return <button key={animation.id} disabled={!isUnlocked} onClick={() => playFlowAnimation(animation.id)} className={`text-left rounded-2xl border p-4 transition-all ${isUnlocked ? 'border-primary/30 bg-primary/5 active:scale-[.98]' : 'border-border bg-surface-2/20 opacity-60'}`}>
