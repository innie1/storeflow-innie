...
function ServiceCatalog({ store, onUpdate, currentUser }: ServicesProps) {
  ...
  /*
   * The advisor and the price run were built for the laundry and left
   * there, on a screen no other trade opens. Neither was ever laundry's:
   * the advisor reads sales, which every trade has, and the run is just a
   * way through a long list. A barber with fourteen cuts to price had the
   * same problem and none of the help.
   */
  const runItems: PriceRunItem[] = services.map(service => ({ key: service.id, name: service.name, price: Number(service.sellingPrice) || 0 }));
  const saveRunPrice = (item: PriceRunItem, name: string, price: number) => {
    const updated = updateProduct(store, item.key, { name, sellingPrice: price }, currentUser?.name, currentUser?.role);
  ...
  return (
    <div className="px-4 py-4 max-w-lg mx-auto pb-24">
      <div className="flex items-center justify-between mb-4"><div><h2 className="font-display font-black text-xl text-foreground">Services</h2><p className="text-xs text-muted-foreground">Set pricing, availability and how the service is delivered.</p></div><div className="flex items-center gap-2 shrink-0">{services.length >= 3 && <button onClick={() => setRunThrough(true)} className="px-3 py-2 rounded-xl bg-surface-2 border border-border font-display font-bold text-xs">Set all prices</button>}<button onClick={openNew} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-primary text-primary-foreground font-display font-bold text-sm"><Plus className="w-4 h-4" /> Add</button></div></div>
      {/* What each service actually earns, and what to do about it. Sits with
          the prices, which is where somebody is already thinking about them. */}
      {services.length > 0 && <PricingAdvisor store={store} onUpdate={onUpdate} />}
      {runThrough && <PriceRun heading={template.name} items={runItems} itemNoun={template.labels.offeringNoun} onSave={saveRunPrice} onClose={() => setRunThrough(false)} />}
      ...
      {confirmDeleteId && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 animate-fade-in px-5" onClick={() => setConfirmDeleteId(null)}><ScrollLock /><div className="w-full max-w-sm bg-background rounded-2xl p-5" onClick={e => e.stopPropagation()}><p className="font-display font-bold text-sm text-foreground">Remove this service?</p><p className="text-xs text-muted-foreground mt-1">Customers won't be able to use it anymore.</p><div className="flex gap-3 mt-4"><button onClick={() => setConfirmDeleteId(null)} className="flex-1 py-2.5 rounded-xl bg-surface-2 border border-border font-display font-semibold text-sm">Cancel</button><button onClick={() => remove(confirmDeleteId)} className="flex-1 py-2.5 rounded-xl bg-destructive text-destructive-foreground font-display font-bold text-sm">Remove</button></div></div></div>}
    </div>
  );
}
