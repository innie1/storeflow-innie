...
export default function SimpleModeHome({ store, setStore, currentUser, onNavigate }: SimpleModeHomeProps) {
  ...
  if (store.category === 'games' || store.storeType === 'games' || businessType === 'games') {
    return <GamesDashboard store={store} onUpdate={setStore} onGoToSettings={() => onNavigate('games-settings')} />;
  }
  ...
  const handleCreateProduct = (name: string, sellingPrice: number, costPrice: number, quantity: number): Product => {
    const newProduct: Product = { id: generateId(), name, costPrice: costPrice || 0, sellingPrice, quantity: Math.max(0, quantity || 0), category: store.storeType || 'others', addedAt: new Date().toISOString(), source: 'voice_sale', needsStockSetup: !costPrice };
    const updated: StoreData = { ...store, products: [...store.products, newProduct] };
    saveStore(updated); setStore(updated); playProductAddedSound(); showToast(`${name} added`, 'success');
    return newProduct;
  };
  ...
  const handleSaveAlias = (productId: string, alias: string) => {
    ...
    if (!clean) return;
    const updated: StoreData = { ...store, products: store.products.map(p => p.id !== productId ? p : { ...p, voiceAliases: [...(p.voiceAliases || []), ...(p.voiceAliases || []).some(a => a.toLowerCase() === clean) ? [] : [clean]] }) };
    saveStore(updated); setStore(updated);
  };

  if (store.simpleOnboarding && !store.simpleOnboarding.complete) return <SimpleOnboarding store={store} setStore={setStore} onComplete={() => {}} />;

  const promptProduct = costPricePromptProductId ? store.products.find(p => p.id === costPricePromptProductId) : null;
  return (
    <div className="relative flex flex-col items-center px-3 pt-0 pb-6 max-w-sm mx-auto">
      ...
      <div className="w-full text-center mb-10">
        ...
        <div className="flex items-center justify-center gap-4 mt-2"><p className="text-xs text-muted-foreground"><span className="font-display font-bold text-success">₦{todayProfit.toLocaleString()}</span> profit</p><span className="w-1 h-1 rounded-full bg-border" /><p className="text-xs text-muted-foreground"><span className="font-display font-bold text-foreground">{todayCount}</span> sale{todayCount === 1 ? '' : 's'}</p></div>
        <div className="mt-3 max-w-[220px] mx-auto"><div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1"><span className="uppercase font-bold tracking-wide">{target.period === 'daily' ? "Today's Target" : "This Week's Target"}{target.mode === 'auto' ? ' (Auto)' : ''}</span><span className="font-bold text-foreground">₦{target.progressAmount.toLocaleString()} / ₦{target.targetAmount.toLocaleString()}</span></div><div className="w-full h-1.5 rounded-full bg-surface-2 overflow-hidden"><div className={`h-full rounded-full transition-all ${target.progressPercent >= 100 ? 'bg-success' : 'bg-primary'}`} style={{ width: `${target.progressPercent}%` }} /></div></div>
      </div>
