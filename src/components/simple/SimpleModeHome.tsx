...
import OfflineQueueBanner, { markSaleQueuedIfOffline } from './OfflineQueueBanne' + 'r';
...
export default function SimpleModeHome({ store, setStore, currentUser, onNavigate }: SimpleModeHomeProps) {
  ...
  const handleCreateProduct = (name: string, sellingPrice: number, costPrice: number, quantity: number): Product => {
    const newProduct: Product = {
      id: generateId(), name, costPrice: costPrice || 0, sellingPrice,
      quantity: Math.max(0, quantity || 0), category: store.storeType || 'others',
      addedAt: new Date().toISOString(), source: 'voice_sale', needsStockSetup: !costPrice,
    };
  ...
  if (store.simpleOnboarding && !store.simpleOnboarding.complete) {
    return <SimpleOnboarding store={store} setStore={setStore} onComplete={() => {}} />;
  }
    ...
    <div className="relative flex flex-col items-center px-3 pt-0 pb-6 max-w-sm mx-auto">
      ...
      <div className="w-full text-center mb-10">
        ...
        <div className="flex items-center justify-center gap-4 mt-2">
          <p className="text-xs text-muted-foreground"><span className="font-display font-bold text-success">₦{todayProfit.toLocaleString()}</span> profit</p>
          <span className="w-1 h-1 rounded-full bg-border" />
          <p className="text-xs text-muted-foreground"><span className="font-display font-bold text-foreground">{todayCount}</span> sale{todayCount === 1 ? '' : 's'}</p>
        </div>
        <div className="mt-3 max-w-[220px] mx-auto">
          <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
            <span className="uppercase font-bold tracking-wide">{target.period === 'daily' ? "Today's Target" : "This Week's Target"}{target.mode === 'auto' ? ' (Auto)' : ''}</span>
            <span className="font-bold text-foreground">₦{target.progressAmount.toLocaleString()} / ₦{target.targetAmount.toLocaleString()}</span>
          </div>
          <div className="w-full h-1.5 rounded-full bg-surface-2 overflow-hidden"><div className={`h-full rounded-full transition-all ${target.progressPercent >= 100 ? 'bg-success' : 'bg-primary'}`} style={{ width: `${target.progressPercent}%` }} /></div>
        </div>
      </div>
      <div className="simple-mode-voice-shell">
        <SimpleVoiceSell products={store.products} onConfirmSale={handleConfirmSale} onConfirmMultiSale={handleConfirmMultiSale} onCreateProduct={handleCreateProduct} onSaveAlias={handleSaveAlias} />
        <button onClick={() => setShowSearch(true)} className="simple-mode-search-near-toggle bg-surface-2 border border-border flex items-center justify-center hover:bg-surface-3 active:scale-95 transition-all cursor-pointer shadow-sm" title="Search products, customers, receipts" aria-label="Search products, customers, receipts"><Search className="w-4 h-4 text-foreground/80" /></button>
      </div>
