...
function patchFlowChat(source: string): string {
  ...
  const statePatch = "  const [restockCodeInput, setRestockCodeInput] = useState('');\n  const [isListening, setIsListening] = useState(false);\n  const recognitionRef = useRef<any>(null);\n  const scrollRef = useRef<HTMLDivElement>(null);";
  if (code.includes(stateAnchor)) code = code.replace(stateAnchor, statePatch);
  else if (!code.includes('recognitionRef = useRef<any>')) throw new Error('[flow-messages] Flow voice state anchor missing');
...
`;
  ...
  const formAnchor = '<form className="flex items-center gap-2 px-4 py-3 border-t border-border" onSubmit={e => { e.preventDefault(); const t = input.trim(); if (!t) return; setInput(\'\'); ask(t); }}><input value={input} onChange={e => setInput(e.target.value)} placeholder={addDraft ? \'Answer Flow…\' : \'Tell Flow what to do…\'} className="flex-1 rounded-full border border-border bg-surface-2/40 px-4 py-3 text-sm" /><button type="submit" disabled={!input.trim()} className="w-11 h-11 flex items-center justify-center rounded-full bg-primary text-primary-foreground disabled:opacity-40" aria-label="Send"><Send className="w-4 h-4" /></button></form>';
  const formPatch = '<form className="flex items-center gap-2 px-4 py-3 border-t border-border" onSubmit={e => { e.preventDefault(); const t = input.trim(); if (!t) return; setInput(\'\'); ask(t); }}><input value={input} onChange={e => setInput(e.target.value)} placeholder={addDraft ? \'Answer Flow…\' : supportsFlowMessageOrders(store) ? \'Type or speak a customer order…\' : \'Message Flow…\'} className="flex-1 rounded-full border border-border bg-surface-2/40 px-4 py-3 text-sm" /><button type="button" onClick={startFlowVoiceInput} className={`w-11 h-11 flex items-center justify-center rounded-full border ${isListening ? \'border-destructive bg-destructive/10 text-destructive animate-pulse\' : \'border-border bg-surface-2/40 text-muted-foreground hover:text-primary\'}`} aria-label={isListening ? \'Stop listening\' : \'Speak to Flow\'} title={isListening ? \'Listening… tap to stop\' : \'Speak to Flow\'}>{isListening ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}</button><button type="submit" disabled={!input.trim()} className="w-11 h-11 flex items-center justify-center rounded-full bg-primary text-primary-foreground disabled:opacity-40" aria-label="Send"><Send className="w-4 h-4" /></button></form>';
  if (code.includes(formAnchor)) code = code.replace(formAnchor, formPatch);
  else if (!code.includes('onClick={startFlowVoiceInput}')) throw new Error('[flow-messages] input form anchor missing');
